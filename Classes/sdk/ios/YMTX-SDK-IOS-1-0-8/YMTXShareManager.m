...
@implementation YMTXShareManager
+(YMTXShareManager *)shareManager
{
    static YMTXShareManager *share = nil;
    if (share == nil) {
        share = [[YMTXShareManager alloc]init];
    }
    return share;
}
...
-(void)shareMovieUrlByAPPType:(TLShareType)type url:(NSString *)movieUrl image:(UIImage *)shareImage imageUrl:(NSString *)imageUrl title:(NSString *)title
{
    ...
    if (!image) {
        ...
        if (!image) {
            image = [UIImage imageNamed:@"ymtx_share_default_icon.png"];
        }
        if (!image) {
            NSLog(@"添加一个分享的默认图标以 ymtx_share_default_icon.png 为图片名（一般为app图标）");
            assert(0);
        }
    }
    ...
    switch (type) {
        ...
        case share_type_qzone:
        {
            ...
            if (![QQApi isQQInstalled]) {
                if ([[YMTXShareManager shareManager]IsLanguageChinese]) {
        ...
        case share_type_fb:
        {
          ...
          SLComposeViewController  *slComposerSheet = [SLComposeViewController composeViewControllerForServiceType:SLServiceTypeFacebook];
            ...
            UIViewController *root = [YMTXRecordManager getSDKRootViewController];
...
-(void)initShareSdk
{
    NSDictionary *setting = [YMTXRequestObject getShareKey];
    ...
    if (setting) {
        NSDictionary *wxDic = YMTXDynamicCast(NSDictionary, [setting objectForKey:@"WX"]);
        if (wxDic) {
            wxkey = [wxDic objectForKey:@"share_key"];
            description = [wxDic objectForKey:@"description"];
        }
        NSDictionary *wbDic = YMTXDynamicCast(NSDictionary, [setting objectForKey:@"WB"]);
        if (wbDic) {
            wbkey = [wbDic objectForKey:@"share_key"];
        }
        NSDictionary *qqDic = YMTXDynamicCast(NSDictionary, [setting objectForKey:@"QQ"]);
        if (qqDic) {
            qqkey = [qqDic objectForKey:@"share_key"];
        }
    }
#ifdef TLWXApi
    ...
    switch (resp.type)
    {
        case ESENDMESSAGETOQQRESPTYPE:
        {
            ...
            if (sendResp.result.integerValue == 0) {
                ...
                [YMTXRequestObject requestVideoShare:share_type_qq movieId:currentShareVideoId];
            }else{
                if([[YMTXShareManager shareManager] IsLanguageChinese]){
                    message = @"\n分享失败";
                }else{
            ...
            [alert show];
            [alert release];
            [[YMTXShareManager shareManager] performSelector:@selector(dismissPopView:) withObject:alert afterDelay:2.f];
            break;
        }
        default:
        {
...
-(void)onResp:(BaseResp*)resp
{
    if([resp isKindOfClass:[SendMessageToWXResp class]])
    {
        ...
        if (resp.errCode == WXSuccess) {
            ...
            if (isShareWxGroup) {
                [YMTXRequestObject requestVideoShare:share_type_wx_group movieId:currentShareVideoId];
            }else{
                [YMTXRequestObject requestVideoShare:share_type_wx movieId:currentShareVideoId];
            }
        }else{
            if([self IsLanguageChinese]){
                message = @"\n分享失败";
...
- (void)didReceiveWeiboResponse:(WBBaseResponse *)response
{
    if ([response isKindOfClass:WBSendMessageToWeiboResponse.class])
    {
        ...
        if (response.statusCode >=0) {
            ...
            [YMTXRequestObject requestVideoShare:share_type_xl movieId:currentShareVideoId];
        }else{
            if([self IsLanguageChinese]){
                message = @"\n分享失败";
            }else{
