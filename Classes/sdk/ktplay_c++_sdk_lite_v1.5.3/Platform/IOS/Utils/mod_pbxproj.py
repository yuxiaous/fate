...
class XCBuildConfiguration(PBXType):
    ...
    def mod_buildSettings(self, setting, toValue):
        modified = False
        base = 'buildSettings'
        key = setting
        ...
        if key not in self[base]:
        	self[base][key] = toValue
        	modified = True
        elif self[base][key] != toValue:
        	self[base][key] = toValue
        	modified = True
...
class XcodeProject(PBXDict):
    ...
    def mod_buildSettings(self, setting, toValue):
        ...
        for b in build_configs:
            if b.mod_buildSettings(setting, toValue):
                self.modified = True        
    ...
    def remove_group(self, id):
    	...
    	print 'begin remove_group id = ' + id
    	
    	gp_obj = self.get_obj(id)
    	print '=====obj ===='
    	print gp_obj
    	
    	if id in self.objects:
    		for child in gp_obj['children']:
    			print '----------child---------------'
    			print child
    			
    			if (self.get_obj(child).get('isa') == 'PBXGroup'):
    				print 'child-g = ' + child
    				self.remove_group(child)
    			elif (self.get_obj(child).get('isa') == 'PBXFileReference'):
    				print 'child-r =' + child
    				self.objects.remove(child)
    				# 
    				srcBuildPhases = [s for s in self.objects.values() if s.get('isa') == 'PBXSourcesBuildPhase']
    				for sp in srcBuildPhases:
    					sp['files'] = [x for x in sp['files'] if self.get_obj(x).get('fileRef') in self.objects]
    					
    				resBuildPhases = [r for r in self.objects.values() if r.get('isa') == 'PBXResourcesBuildPhase']
    				for bp in resBuildPhases:
    					bp['files'] = [y for y in bp['files'] if self.get_obj(y).get('fileRef') in self.objects]
    					
    				frameworkBuildPharses = [f for f in self.objects.values() if f.get('isa') == 'PBXFrameworksBuildPhase']
    				for fp in frameworkBuildPharses:
    					fp['files'] = [z for z in fp['files'] if self.get_obj(z).get('fileRef') in self.objects]
    			else:
    				print 'child other'

       	self.objects.remove(id)
       	self.modified = True
       	print 'end remove_group id'
