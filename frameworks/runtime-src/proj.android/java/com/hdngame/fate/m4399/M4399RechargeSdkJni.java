...
public class M4399RechargeSdkJni {
    ...
    public static void init(String gameKey, String gameName) {
        ...
        SingleOperateCenter.getInstance().init(SdkManagerJni.activity, listerner);
    }

    final static SingleOperateCenter.SingleRechargeListener listerner = new SingleOperateCenter.SingleRechargeListener() {
        ...
        synchronized public boolean notifyDeliverGoods(boolean shouldDeliver, RechargeOrder o) {
            if (shouldDeliver) {
                ...
                onM4399ChargeCallback(0, _order);
                _order = "";
                return true;
            } else {
                System.out.println("单机充值查询到的订单状态不正常，建议不要发放物品");
                onM4399ChargeCallback(1, _order);
                return false;
            }
        }
    };
    ...
    public static native void onM4399ChargeCallback(int result, String order);
}
